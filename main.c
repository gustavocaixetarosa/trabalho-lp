...
void artesArcanistasArcanas(Turma turmas[]){
    ...
    printf("Temos nossos representantes! Pela turma %s, o bruxo %s. Do outro lado, %s da %s! Que venca o mais sabichao. RisosRisos\n\n",
        turmas[turmaA].nome, turmas[turmaA].alunos[alunoA].nome, turmas[turmaB].alunos[alunoB].nome, turmas[turmaB].nome);
...
void zerarVitoriaDesafios(Turma turmas[]){
    for(int i = 0; i < QTD_TURMAS; i++){
        turmas[i].vitoriasDesafios = 0;
    }
}
